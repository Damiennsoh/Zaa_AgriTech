...
async def check_group_readiness(group_id: str) -> Dict[str, Any]:
    ...
    async with pool.acquire() as conn:
        group = await conn.fetchrow(
            "SELECT * FROM selling_groups WHERE id = $1",
            group_id
        )
        if not group:
        ...
        total = await conn.fetchval(
            "SELECT COALESCE(SUM(quantity_contributed), 0) FROM group_members WHERE group_id = $1",
            group_id
        )
        ...
        return {
            ...
            "target_quantity": group.get("target_quantity", 500),
            "is_ready": total >= group.get("target_quantity", 500),
            "status": group["status"]
        }
